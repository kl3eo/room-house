...
impl SubstrateCli for Cli {
	fn impl_name() -> String { "xTER Room-House".into() }
	...
	fn support_url() -> String { "https://github.com/kl3eo/roomhouse/issues/new".into() }

	fn copyright_start_year() -> i32 { 2017 }

	fn executable_name() -> String { "roomhouse".into() }

	fn load_spec(&self, id: &str) -> std::result::Result<Box<dyn sc_service::ChainSpec>, String> {
		let id = if id == "" {
			let n = get_exec_name().unwrap_or_default();
			["roomhouse", "kusama", "westend", "rococo"].iter()
				.cloned()
		...
		Ok(match id {
			...
			"roomhouse" => Box::new(service::chain_spec::roomhouse_config()?),
	...
	fn native_runtime_version(spec: &Box<dyn service::ChainSpec>) -> &'static RuntimeVersion {
		...
		&service::roomhouse_runtime::VERSION
	}
}
...
pub fn run() -> Result<()> {
	...
	match &cli.subcommand {
		...
		Some(Subcommand::Benchmark(cmd)) => {
			...
			// else we assume it is roomhouse.
			Ok(runner.sync_run(|config| {
				cmd.run::<service::roomhouse_runtime::Block, service::PolkadotExecutor>(config)
					.map_err(|e| Error::SubstrateCli(e))
			})?)
		},
		...
		Some(Subcommand::TryRuntime(cmd)) => {
			...
			runner.async_run(|config| {
				Ok((cmd.run::<
					service::roomhouse_runtime::Block,
