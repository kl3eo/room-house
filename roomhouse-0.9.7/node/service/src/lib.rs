...
>
{
	...
	let service::PartialComponents { client, backend, import_queue, task_manager, .. }
		= new_partial::<roomhouse_runtime::RuntimeApi, PolkadotExecutor>(config, jaeger_agent, None)?;
...
pub fn build_light(config: Configuration) -> Result<(
	...
	new_light::<roomhouse_runtime::RuntimeApi, PolkadotExecutor>(config)
}
...
pub fn build_full(
	...
	new_full::<roomhouse_runtime::RuntimeApi, PolkadotExecutor>(
		config,
		is_collator,
		grandpa_pause,
		disable_beefy,
