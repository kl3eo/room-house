...
function checkMessage(message) {
  ...
  {
  error = "The message is longer than 800 characters.\n";
  }
...
// Get messages from database each N seconds, but no longer then (ajax_counter by N)
var ajax_counter = 0;
...
function displayChatData(reqObj) {
  ...
  var message_nodes = typeof xmldoc === 'undefined' || xmldoc === null ? [] : xmldoc.getElementsByTagName('msg');
...
function sendMessage() {
  ...
  var room = document.getElementById('room').value;
  var apo = new RegExp('\'','ig');
  message = message.replace(apo,'');
 
//console.log('name is ', name, 'token is ', token, 'url is ', url,'mess is ', message);
...
//console.log('checked 1!');
  ...
  senderXMLHttpObj.send('room='+room+'&token='+token+'&name='+encodeURIComponent(name)+'&url='+encodeURIComponent(url)+'&message='+encodeURIComponent(message));
...
function startChat() {
  ...
  submit.onclick = function () {sendMessage();getChatData();}
